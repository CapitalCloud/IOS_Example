...
#import "CapitalCloudImageDownloader.h"
    ...
    didFinishPickingMediaWithInfo: (NSDictionary *) info
{
    ...
    if ([mediaType isEqualToString:(NSString*)kUTTypeMovie])
    {
        ...
        [[CapitalCloudHelper uploaderManager] upload:movieURL paramters:[[NSDictionary alloc] init] error:&error];
...
- (IBAction)fetchVideos
{
    ...
    CapitalCloudAPIClient *client = [CapitalCloudHelper apiClient];    // create a (non-main) queue to do fetch on
    dispatch_queue_t fetchQ = dispatch_queue_create("capitalCloud video fetcher", NULL);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CapitalCloudVideo *video = self.videos[indexPath.row];
...
- (void)startIconDownload:(CapitalCloudVideo *)video forIndexPath:(NSIndexPath *)indexPath
{
    CapitalCloudImageDownloader *imageDownloader = [self.imageDownloadsInProgress objectForKey:indexPath];
    if (imageDownloader == nil)
    {
        imageDownloader = [[CapitalCloudImageDownloader alloc] init];
...
- (void)loadImagesForOnscreenRows
{
    if ([self.videos count] > 0)
    {
        ...
        {
            CapitalCloudVideo *video = [self.videos objectAtIndex:indexPath.row];
            if (!video.thumbnailImage)
            {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    else if ([segue.destinationViewController isKindOfClass:[VideoUploadListViewController class]])
    {
        ...
        iuvc.uploadList = [[CapitalCloudHelper uploaderManager] getUploaderList];
    }
}
